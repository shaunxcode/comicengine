var artisan = (function(window, undefined) {
	...
	*/
	...
	return {
		...
		drawImage: function(target, src, placex, placey, width, height, rotate, center, alpha, fill_color, line_width, stroke_color, shadow_blur, shadow_color, shadow_offset_x, shadow_offset_y){
			var targeted = document.getElementById(target);
			var target_width = targeted.width;
			var target_height = targeted.height;
			if (src) {
				...
				case "CANVAS":
					...
					this_image.onload = function(){
						...
						if(rotate) {
						//	artisan.drawCircle(target, placex + (width / 2), placey + (height / 2), 3, 'random'); 
							
							context.save();
							//context.translate(0, 0);
							context.translate(placex + (width / 2), placey + (height / 2));
							//artisan.drawCircle(target, placex + (width / 2), placey + (height / 2), 3, 'random'); 
							context.rotate(rotate * Math.PI/180);
							//context.translate(-(placex + (width /2)), -(placey + (height / 2)));
							
						    context.drawImage(this_image, -(width / 2), -(height / 2), width, height) // , 0, 0, width, height);
							
							//context.drawImage(this_image, 10, 30, width, height);
						//	context.drawImage(this_image, placex - (width * 2), placey - (height * 2), width, height);
							context.restore();
						}  else {	
